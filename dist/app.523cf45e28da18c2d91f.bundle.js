...
var md = __webpack_require__(2)({ 
    html: true,
    typographer: true
}).use(__webpack_require__(1), {
            imageClass: 'un-named_class',
...
xhrPrms('/data/', 'document').then(function(response) {
    ...
    for(var i = 0; i < linksA.length; i++) {
        if (linksA[i].href.match(/\.md$/)) {
            prms = xhrPrms(linksA[i].href, 'text')
            prmsArr.push(prms);
            //console.log(linksA[i].href);
        } else {
            //console.log("Bad Links: " + linksA[i].href);
        }
    ...
    Promise.all(prmsArr).then(values => { 
        ...
        for(var i = 0; i < values.length; i++) {
            var mdResult = md.render(values[i]);
            $("div#content").append(mdResult);
        }
    });
...
var mdResult = md.render();

$("div#content").html(mdResult);
